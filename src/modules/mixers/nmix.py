...
class Mixer(nn.Module):
    def __init__(self, args, abs=True):
        ...
        self.input_dim = self.state_dim = int(np.prod(args.state_shape))     #计算状态向量的总长度或大小，并将这个值设置为类的 input_dim 和 state_dim 属性

        self.abs = abs # monotonicity constraint
        self.qmix_pos_func = getattr(self.args, "qmix_pos_func", "abs")    #QMIX没有这个参数
    ...
    def forward(self, qvals, states):
        # reshape
        b, t, _ = qvals.size()    #b为batch_size, t为episode_length, _为n_agents(智能体数量)
        
        qvals = qvals.reshape(b * t, 1, self.n_agents)
        states = states.reshape(-1, self.state_dim)

        # First layer
        w1 = self.hyper_w1(states).view(-1, self.n_agents, self.embed_dim) # b * t, n_agents, embed_dim
