...
class EpsilonGreedyActionSelector():
    ...
    def select_action(self, agent_inputs, avail_actions, t_env, test_mode=False):
        ...
        # mask actions that are excluded from selection 屏蔽从选择中排除的操作
        masked_q_values = agent_inputs.clone()    #创建该张量的一个深拷贝
        masked_q_values[avail_actions == 0] = -float("inf")  # should never be selected! 返回一个新视图（view）或子张量（subtensor），其中只包含masked_q_values中那些avail_actions为0的位置的元素，将Q值设置为负无穷大。
